...
function ClearRoomContent() {
    ...
    $(function () {
        $(".login-content").on("click", ".panel_item", e), $(".container").on("click", r), $(".role-list").on("click", ".role-item", function () {
            ...
            }), $(".channel-box").on("click", "span", m), $(".combat-commands").on("click", ".pfm-item", j.Perform), $(".sender-box").on("keyup", I), $(".room_items").on("click", ".room-item", T.selectItem), $(".right-bar,.bottom-bar").on("click", ".tool-item", d), $(".map-panel").on("click", function () {
                this.last_click = this.last_click || 0, 500 < Date.now() - this.last_click ? this.last_click = Date.now() : M.show("map")
            ...
            function () {
                if (!h("p")) return $("#login_panel").show();
                l()
            }()
    });
    var n, t, o = {
        ToUpdate: function () {
            HideAndShow("#pwd_panel"), L.UserAPI.GetPhone(function (e) {
                e ? ($("#pwd_phone").prop("disabled", !0).val(e), $("#pwd_bind").show()) : ($("#pwd_phone").prop("disabled", !1).val(""), $("#pwd_bind").hide())
            })
        },
        ResetPwd: function () {
            ...
            var s = $("#reset_no").val();
            if (!s) return w($("#reset_no").parent(), "请输入你接收到的六位验证码");
            if (!/^\d{6}$/.test(s)) return w($("#reset_no").parent(), "请输入六位数字的验证码");
            var i = $("#reset_pwd1").val();
            if (!i) return w("#reset_pwd1", "请输入你的新密码");
            var n = $("#reset_pwd2").val();
            return n ? n.length < 6 || 20 < n.length ? w("#update_pwd2", "密码长度在6到20之间") : n != i ? w("#reset_pwd2", "两次密码输入不一致") : (k("正在修改密码", "#reset_panel"), void L.UserAPI.ResetPasswordByPhone(e, t, s, i, function (e) {
                e ? (w("#reset_pwd2", e), HideAndShow("#reset_panel")) : HideAndShow("#login_panel")
            })) : w("#reset_pwd2", "请重复输入你的新密码")
        },
        BindPhone: function () {
            HideAndShow("#bind_panel"), L.UserAPI.GetPhone(function (e) {
                e ? ($("#phone_no").prop("disabled", !0).val(e), $("#phone_no").prev().html("你已绑定手机，再次验证会取消绑定"), $("#phone_no").parent().next().find("span:last()").html("解除绑定")) : ($("#phone_no").prop("disabled", !1).val(""), $("#phone_no").prev().html("你要绑定的手机"), $("#phone_no").parent().next().find("span:last()").html("绑定"))
            })
        },
        CheckValid: function () {
            var e = $("#phone_no"),
                t = "";
            if (!e.is(":disabled")) {
                if (!(t = e.val())) return w("#phone_no", "请输入你的帐号绑定的手机号码");
                if (!/^(13[0-9]|14[579]|15[0-3,5-9]|16[6]|17[0135678]|18[0-9]|19[89])\d{8}$/.test(t)) return w("#reset_phone", "手机号码格式错误")
            }
            var s = $("#phone_valid").val();
            return s ? /^\d{6}$/.test(s) ? void L.UserAPI.BindPhone(s, t, function (e) {
                e ? (w($("#phone_valid").parent(), e), HideAndShow("#bind_panel")) : HideAndShow("#role_panel")
        ...
        UpdatePwd: function () {
            var e, t = $("#update_pwd1").val(),
                s = $("#update_pwd2").val(),
                i = $("#update_pwd3").val();
            if (t.length < 6 || 20 < t.length) return w("#update_pwd1", "密码长度在6到20之间");
            if (s.length < 6 || 20 < s.length) return w("#update_pwd2", "密码长度在6到20之间");
            if (i != s) return w("#update_pwd3", "两次密码输入不一致");
            if ($("#pwd_bind").is(":visible")) {
                if (!(e = $("#pwd_no").val())) return w($("#pwd_no").parent(), "请输入你接收到的六位验证码");
                if (!/^\d{6}$/.test(e)) return w($("#pwd_no").parent(), "请输入六位数字的验证码")
            }
            k("正在修改密码", "#pwd_panel"), L.UserAPI.ChangePassword(t, s, e, function (e) {
                e.code ? HideAndShow($("#role_panel")) : (w("#update_pwd1", e.message || "修改失败"), HideAndShow("#pwd_panel"))
            })
        },
        LoginIn: function () {
            var e = $("#login_name").val().toLowerCase(),
                t = $("#login_pwd").val();
            return e ? /^[a-z0-9]{5,15}$/.test(e) ? t ? t.length < 6 || 20 < t.length ? w("#login_pwd", "密码长度在6到20之间") : (k("正在登录", "#login_panel"), void L.UserAPI.Login(e, t, function (e) {
                e.code ? l() : (w("#login_name", e.message || "登陆失败"), HideAndShow("#login_panel"))
        ...
        SelectServer: function () {
            if (n) {
                ...
                if (!(0 <= e && e < n.length)) return F.Show({
        ...
        AddRole: function () {
            if (4 < $(".role-list>.role-item").length) return F.Show({
                content: "你只能最多创建五个角色"
            });
        ...
        Regist: function () {
            ...
            L.UserAPI.Regist(s, function (e) {
                e.code ? l() : (w("#regist_name", e.message || "注册失败"), HideAndShow("#regist_panel"))
            })
        }
    };
    ...
    function a() {
        ...
        if (!t.is(":disabled")) {
            ...
            if (e.is(":disabled")) s = "";
                ...
                if (!s) return w(e, "请输入你的帐号绑定的手机号码");
                if (!/^(13[0-9]|14[579]|15[0-3,5-9]|16[6]|17[0135678]|18[0-9]|19[89])\d{8}$/.test(s)) return w(e, "手机号码格式错误")
            }
            L.UserAPI.SendValidateCode(s, function (e) {
                    e || w(t.parent(), "验证码发送失败")
                }), t.prop("disabled", !0), t.html("120秒后重新发送"),
                function e(t, s) {
    ...
    function l() {
        if (!n) return k("正在获取服务器列表"), void L.UserAPI.GetServer(function (e) {
            e && "string" != typeof e ? (n = e || [], function () {
                ...
                for (var e = [], t = ["武神传说", "武神传说", "武神传说", "武神传说"], s = 0; s < n.length; s++) n[s].IsTest && "?test" != window.location.search && "localhost" != window.location.hostname ? n.splice(s, 1) : (e.push("<li class='role-item"), 0 == s && e.push(" select"), e.push("' index='" + s + "'>"), e.push(t[s]), e.push("&nbsp;&nbsp;"), e.push(n[s].Name), n[s].IsRcd && e.push("<span style='color:red;font-size:0.5rem;line-height:2rem;height:2rem;'>&nbsp;（推荐）</span>"), e.push("</li>"));
            ...
            if ("_" == s[0]) {
                ...
                switch (i[0]) {
                    ...
                    case "_close":
                        ...
                        break;
                    case "_hide":
                        Storage.ban_user(i[1])
                }
            } else c(s);
            return !1
        }
        return p && (t.closest(".chat-panel").length || ($(".chat-panel").addClass("hide"), p = !1)), F.Close(), !1
    }
    ...
    function g(e, t) {
        _ || (s = e, console.log("重新连接", null == i ? "未连接" : "已连接"), i = new G(e.IP, e.Port), _ = !0, i.OnError = function (e, t) {
    ...
    function S(e) {
        f || (f = $("<pre></pre>").appendTo(u)), 1e3 < b && (b = 0, f.empty()), f.append(e + "\n"), b++, T.contentScroll && (u[0].scrollTop = 99999)
    }
            ...
            case "name":
                $("#reg_name").val(function (e, t) {
                    ...
                    if (2 == t) {
                        ...
                        i % 2 == 1 && (i -= 1), s.push(R[i++]), s.push(R[i])
    ...
    var T = {
            ...
            contentScroll: !0,
            clear: function () {
                M.pack.items = null, M.skills.items = null, this.state(null)
            },
            init: function () {
                T.itemsElement = $(".room_items"), (u = $(".content-message")).on("mousedown", function () {
                    T.contentScroll = !1
            ...
            cross: function (e) {
                ...
                t && (i.ChangeServer = !0, i.Close(), M.pack.items = null, "duizhan" == e.cross_type && (M.skills.items = null, M.skills.isShow = !1), console.log("重新连接到", t.Name), e.pid || T.die({
                    relive: !0
                }), g(t, e.pid))
            },
            roles: function (e) {
            ...
            login: function (e) {
                T.player || HideAndShow(".container"), T.player = e.id, q.load(e.setting), t && (c(t), t = null)
            },
            selectItem: function (e) {
                ...
                if (s) {
                    if (s == T.player) {
                        ...
                        return M.team.items && M.team.items.length && (n.push({
                            cmd: "team out",
                            name: "退出队伍"
                        }), M.team.isCap && (n.push({
                            cmd: "team dismiss",
            ...
            itemadd: function (e) {
                if (!q.off_plist || !e.p || e.id == T.player) {
                    var t, s = e;
                    t = q.item_firstme && s.id == T.player ? $(T.create_roomitem(s)).prependTo(T.itemsElement) : $(T.create_roomitem(s)).appendTo(T.itemsElement), j.STATUS[e.id] && T.itemremove(e), j.AppendStatusItem(s.id, t.find(".item-status-bar"), s.status)
                }
            },
            items: function (e) {
                ...
                for (var t = 0; t < e.items.length; t++) {
                    var s, i = e.items[t];
                    if (i)
                        if (!q.off_plist || !i.p || i.id == T.player) s = q.item_firstme && i.id == T.player ? $(T.create_roomitem(i)).prependTo(T.itemsElement) : $(T.create_roomitem(i)).appendTo(T.itemsElement), j.AppendStatusItem(i.id, s.find(".item-status-bar"), i.status)
                }
                T.cur_room = e
            },
            get_hpnum: function (e, t) {
                var s = e / t;
                return .8 < s ? "<hiy>" + e + "</hiy>" : .5 < s ? "<yel>" + e + "</yel>" : .2 < s ? "<red>" + e + "</red>" : "<hir>" + e + "</hir>"
            },
            create_roomitem: function (e) {
                var t = [];
                return t.push("<div class='room-item' itemid='" + e.id + "'>"), e.max_hp && (t.push('<div class="item-status"'), j.IsShow && !q.off_hp || t.push(' style="display:none;"'), t.push(">"), t.push('<div class="progress hp"><div class="progress-bar" max="' + e.max_hp + '"  style="width:' + T.countwidth(e.hp, e.max_hp) + '%"></div></div>'), t.push('<div class="progress mp"><div class="progress-bar" max="' + e.max_mp + '"   style="width:' + T.countwidth(e.mp, e.max_mp) + '%"></div></div>'), t.push("</div>")), t.push("<span class='item-status-bar'>"), t.push("</span>"), t.push("<span class='item-name'>"), t.push(e.name), q.show_hpnum && e.max_hp && t.push('<span class="progress-num">[' + this.get_hpnum(e.hp, e.max_hp) + "<nor>/</nor><hiy>" + e.max_hp + "</hiy>]</span>"), t.push("</span>"), t.push("</div>"), t.join("")
            },
            room: function (e) {
                if ($(".room_items").html(""), $(".room-name").html(e.name), $(".room_desc").html(e.desc), T.room_name = e.name, !q.keep_msg && f ? (f.remove(), f = null, b = 0) : q.keep_msg && S("你来到了" + e.name + "。"), T.room_path != e.path) {
            ...
            item: function (e) {
                if (S(e.desc), e.commands) {
                    ...
                    if (t.push("</div>"), q.show_command && j.STATUS[e.id]) {
                        T.itemsElement.find(".item-commands").remove();
                        var i = j.STATUS[e.id].elem.parent();
                        return $(t.join("")).insertAfter(i), u[0].scrollTop = 99999
                    }
                    S(t.join(""))
                }
            },
            ...
            combat: function (e) {
                e.start && (1 != q.auto_showcombat || j.IsShow || j.Show(), 1 == q.auto_hideroom && (q.hide_roomdesc || $(".room_desc").hide())), e.end && 1 == q.auto_hideroom && (q.hide_roomdesc || $(".room_desc").show())
            },
            state: function (e) {
                if (e && e.state) {
            ...
            msg: function (e) {
                var t = M.channel.createElement(e, !q.no_spmsg);
                if (t)
                    if (q.no_spmsg) S(t);
                    else {
                        T.ChannelElement || (T.ChannelCount = 0, T.ChannelElement = $("<div class='channel'><pre></pre></div>").insertBefore(u), T.ChannelPro = T.ChannelElement.find("pre"), T.ChannelElement.on("click", M.channel.show.bind(M.channel)), T.ChannelElement.on("mousedown", M.channel.endScroll), T.ChannelElement.on("mouseup", M.channel.beginScroll));
                        var s = M.channel.isBottom();
                        1 == M.channel.datas.length && T.ChannelPro.empty(), T.ChannelPro.append(t), s && M.channel.scrollBottom()
                    }
            },
        ...
        j = {
            ...
            Show: function () {
                ...
                this.Actions || c("actions"), j.IsShow = !0, q.off_hp || $(".room-item>.item-status").show(), $(".combat-panel").removeClass("hide"), $(".right-bar")[0].style.bottom = $(".combat-panel").height() + $(".bottom-bar").height() + "px"
            },
            Hide: function () {
                j.IsShow = !1, q.off_hp || $(".room-item>.item-status").hide(), $(".combat-panel").addClass("hide"), $(".right-bar")[0].style.bottom = null
            },
                ...
                for (var e = 100 * j.ani_time / j.distime, t = 0; t < j.Skills.length; t++) {
                    for (var s = j.Skills[t], i = e, n = 0; n < j.dis_pfms.length; n++)
                        if (j.dis_pfms[n].id == s.id && j.dis_pfms[n].distime) {
                            ...
                            break
                        } 0 < i ? (i < 0 && (i = 0), s.shadow.style.left = 100 - i + "%", s.shadow.style.display = "block") : (s.shadow.style.display = "none", s.shadow.style.left = "0px")
                }
            ...
            UpdaeBar: function (e, t, s) {
                ...
                if (null != i) {
                    ...
                    if (e["max_" + t] ? (n = e["max_" + t], o.attr("max", n)) : n = parseInt(o.attr("max")), q.show_hpnum && "hp" == t && s.find(".progress-num").html("[" + T.get_hpnum(i, n) + "<nor>/</nor><hiy>" + n + "</hiy>]"), o.css("width", j.CountWidth(i, n) + "%"), q.show_damage && e.damage && e.id != T.player) {
                        var a = 0;
                        a = -1 == e.damage ? parseInt(1e3 * (n - i) / n) / 10 : parseInt(1e3 * e.damage / n) / 10, (o = s.find(".item-damage")).length || (o = $('<span class="item-damage">[<hiy>0%</hiy>]<span>').appendTo(s.find(".item-name"))), o.html("[<hiy>" + a + "%</hiy>]")
                    }
                }
            },
            CountWidth: function (e, t) {
    ...
    var A = {
            ...
            CreateExitsMap: function (e, t, s) {
                ...
                var r = [];
                for (var h in r.push('<svg style="margin-left:-2em" height="' + n + '" width="' + t + '">'), r.push('<rect x="' + o + '" y="' + a + '"  fill="dimgrey" stroke-width="1" stroke="gray" '), r.push('width="60" height="20"></rect>'), r.push(' <text x="' + (o + 30) + '" y="' + (a + 14) + '"  text-anchor="middle" style="font-size:12px;" '), this.pushName(r, s, !0), e) {
                    var c, m, d;
                    switch (h) {
                        case "west":
                        case "westup":
                    ...
                    var p = e[h];
                    "south_2" == h ? h = "down" : "north_2" == h && (h = "up"), r.push('<rect x="' + d[0] + '" y="' + d[1] + '" dir="' + h + '" fill="#232323" stroke-width="1" stroke="gray" '), r.push('width="60" height="20"></rect>'), r.push(' <text x="' + (d[0] + 30) + '" y="' + (d[1] + 14) + '" dir="' + h + '" text-anchor="middle" style="font-size:12px;"'), this.pushName(r, p, !1), c && (r.push('<line  stroke="gray" '), r.push(" x1='" + c[0] + "' y1='" + c[1] + "' x2='" + m[0] + "' y2='" + m[1] + "'"), -1 < h.indexOf("up") || -1 < h.indexOf("down") ? (r.push(" stroke-dasharray='5,5'"), r.push(" stroke-width='10'")) : r.push(" stroke-width='1'"), r.push("></line >"))
                }
                    ...
                    if (A.MapWidth < c && (r = (c - A.MapWidth) / 2, A.MapWidth = c), A.MapHeight = 50 * (i.maxY + o + 1), !(A.MapWidth < 0 || A.MapHeight < 0)) {
                        ...
                        for (var m = 0; m < e.length; m++) {
                            ...
                            var d = (e[m].p[0] + n) * a + r + 20,
                                p = 50 * (e[m].p[1] + o) + 20;
                            s.push("x='" + d + "' y='" + p + "'"), s.push(' fill="dimgrey" stroke-width="1" stroke="gray" '), s.push('width="60" height="20"></rect>');
                            var u = e[m].exits;
                            if (u)
                                for (var f = 0; f < u.length; f++) {
                                    ...
                                    switch (RegExp.$1) {
                                        case "w":
                                            v = [d - 40 - a * (g - 1), p + 10], _ = [d, p + 10];
                                            break;
                                        case "e":
                                            v = [d + 60, p + 10], _ = [d + a + a * (g - 1), p + 10];
                                            break;
                                        case "s":
                                            v = [d + 30, p + 20], _ = [d + 30, p + 50 + 50 * (g - 1)];
                                            break;
                                        case "n":
                                            v = [d + 30, p], _ = [d + 30, p - 30 - 50 * (g - 1)];
                                            break;
                                        case "nw":
                                            v = [d - g * a + 60, p - 50 * g + 20], _ = [d, p];
                                            break;
                                        case "ne":
                                            v = [d + 60, p], _ = [d + g * a, p - 30];
                                            break;
                                        case "se":
                                            v = [d + 60, p + 20], _ = [d + g * a, p + 50 * g];
                                            break;
                                        case "sw":
                                            v = [d, p + 20], _ = [d - 40 - a * (g - 1), p + 50 * g]
                                    }
                            ...
                            s.push(' <text x="' + (d + 30) + '" y="' + (p + 14) + '" text-anchor="middle" style="font-size:12px;" '), this.pushName(s, e[m].n, !0)
                        }
            ...
            pushName: function (e, t, s) {
                ...
                i ? (e.push('  fill="' + this.GetColor(i[1]) + '"'), e.push(">" + i[2] + "</text>")) : (e.push(' fill="'), e.push(s ? "#232323" : "dimgrey"), e.push('">' + t + "</text>"))
            },
            ...
            UpdateMap: function (e, t) {
                var s = A.Buffer[e];
                if (s) {
                    if (!t.id) return A.Buffer[e] = null, void(this.CurMapID == e && (this.CurMapID = null));
                    for (var i = 0; i < s.length; i++)
                        if (s[i].id == t.id) {
                            s[i].n = t.n || s[i].n, s[i].p = t.p || s[i].p;
                            break
                        } e == this.CurMapID && A.ShowMap(s, e)
                }
            }
        },
        M = {
            ...
            show: function (e, t) {
                if (e)
                    if (t) this[e].onData(t);
                        ...
                        this.curItem && e != this.curItem && (M[M.curItem].close && M[M.curItem].close(), M[M.curItem].isShow = !1, M.contentElement.empty()), this.init(), this.curItem = e, this[e].show(t)
                    }
            },
            select: function (e) {
                if (this.isShow && e == this.curItem) return this.hide();
                this.curItem && e != this.curItem && (M[M.curItem].close && M[M.curItem].close(), M[M.curItem].isShow = !1, M.contentElement.empty()), this.init(), this.curItem = e
            },
            init: function () {
                this.isShow || (this.isInit || (this.contentElement = $(".dialog>.dialog-content"), this.titleElement = $(".dialog>.dialog-header>.dialog-title"), this.iconElement = $(".dialog>.dialog-header>.dialog-icon"), this.footerElement = $(".dialog>.dialog-footer").on("click", ".footer-item", M.footerClick), this.hiddenElement = $(".hidden-item"), this.element = $(".dialog"), $(".dialog>.dialog-header>.dialog-close").on("click", M.hide), this.isInit = !0), $(".content-room").addClass("hide"), this.element.removeClass("hide"), this.isShow = !0)
            },
            hide: function () {
                M[M.curItem].hide && 0 == M[M.curItem].hide() || (M.isShow = !1, $(".content-room").removeClass("hide"), M.element.addClass("hide"))
            },
            footerClick: function () {
                var e = $(this);
                if (!e.is(".select")) {
                    var t = e.attr("for");
                    e.parent().find(".select").removeClass("select"), e.addClass("select"), M[M.curItem].footerChanged(t)
                }
            },
            title: function (e) {
                M.titleElement.html(e)
            },
            icon: function (e) {
            ...
            score: {
                ...
                selectIndex: 0,
                onData: function (e) {
                    if (console.log(e), this.data = e, this.init_elem(), M.titleElement.html(e.name), M.icon("user"), e.titles) this.titles = e.titles, this.create_titles();
                    else {
                        e.id && e.id != this.uid && (this.uid = e.id, this.uid != T.player ? M.footerElement.find(".footer-item:eq(2)").hide() : M.footerElement.find(".footer-item:eq(2)").show());
                ...
                init_elem: function () {
                    if (M.init(), M.curItem = "score", !this.isShow) {
                        M.footer("");
                        for (var e = 0; e < this.footer.length; e++) {
                            $("<span class='footer-item " + (this.selectIndex == e ? "select" : "") + "' for='" + e + "'>" + this.footer[e][0] + "</span>").appendTo(M.footerElement);
                ...
                close: function () {
                    this.footer[this.selectIndex][1].remove(), M.footer(""), this.isShow = !1
                },
                footerChanged: function (e) {
                    this.data;
                    e = parseInt(e), this.footer[this.selectIndex][1].remove(), this.selectIndex = e;
                    var t = $(this.footer[this.selectIndex][1]).appendTo(M.contentElement.empty());
                    1 == e ? this.uid && T.player != this.uid ? c("score2 " + this.uid) : c("score2") : 2 == e && (this.titles || c("score title"), t.on("click", ".btn-noused", function (e) {
                        var t = $(e.target);
                        t.is("red") && (t = t.parent());
                        for (var s = parseInt(t.attr("index")), i = 0; i < this.titles.length; i++) this.titles[i].use = i == s && !this.titles[i].use;
                        c("title " + s), this.create_titles()
                    }.bind(this)))
                },
                create_titles: function () {
                    for (var e = $(".dialog-titles"), t = [], s = 0; s < this.titles.length; s++) t.push("<div class='title-item'>"), t.push(this.titles[s].title), t.push("<span class='btn-noused' index='"), t.push(s), t.push("'>"), t.push(this.titles[s].use ? "<red>取消</red>" : "使用"), t.push("</span>"), t.push("</div>");
            ...
            map: {
                ...
                show: function () {
                    M.init(), M.footer(""), M.contentElement.append($(".map")), M.icon("map-marker"), M.iconElement.attr("class", "dialog-icon glyphicon glyphicon-map-marker")
                },
                hide: function () {
    ...
    M.skills = {
        ...
        create_footer: function () {
            ...
            t.push("<span class='obj-money'>你目前的技能上限为<HIC>" + this.limit + "</HIC>级</span>"), M.footer(t.join(""))
        },
        updateSkill: function (e) {
            if (this.skills) {
                var t = this.skills[e.id];
                if (!t) return this.addSkill(t);
                if (e.name && (t.name = e.name), e.enable) {
                    if (t.enable_skill) {
        ...
        addSkill: function (e) {
            if (this.items && e) {
                if (this.skills[e.id]) return this.updateSkill(e);
                this.items.push(e), this.skills[e.id] = e, this.items = this.sort_items(this.items), this.createSkillItems(this.items)
            }
        },
        onData: function (e) {
            if (e) {
                if (!this.isShow && M.master.isShow && null == e.exp && !e.item) return M.master.onData(e);
                ...
                if (e.items) {
                    this.title = e.title, M.title(this.title), M.icon("book"), this.items = this.sort_items(e.items), this.skills = {};
        ...
        show: function () {
            this.isShow || (this.isShow = !0, this.element || (this.element = $('<div class="dialog-skills"></div >')), this.element.on("click", ".skill-item", M.skills.item_click), this.element.appendTo(M.contentElement), this.element.removeClass("hide-item"), this.items ? (c("cha none"), M.title(this.title), M.icon("book"), this.create_footer()) : c("cha"))
        },
        ...
        createSkillItem: function (e, t) {
            ...
            return s.push('<span class="skill-level">'), s.push(e.level), s.push("级 / "), s.push(e.exp), s.push("%"), s.push("&nbsp;"), s.push(M.skills.get_lvdesc(e.level)), s.push("</span></div>"), s.join("")
        },
        sort_items: function (e) {
            if (!e || !q.auto_sortitem) return e;
            for (var t = [], s = 0; s < e.length; s++) {
                var i = e[s],
                    n = !1;
                i.s_index || (i.s_index = M.pack.get_index(i));
        ...
        get_lvdesc: function (e) {
            if (e < 1e3) return M.skills.skill_levels[parseInt(e / 50)];
            var t = parseInt((e - 1e3) / 500);
            return 6 < t && (t = 6), M.skills.skill_levels[t + 20]
        },
        ...
        close: M.skills.close,
        createSkillItems: M.skills.createSkillItems,
        createSkillItem: M.skills.createSkillItem,
        updateSkill: M.skills.updateSkill,
        updateSkillItem: M.skills.updateSkillItem,
        showdesc: M.skills.showdesc,
        isEnable: M.skills.isEnable,
        onData: function (e) {
            if (e.desc) return this.showdesc(e);
            if (e.id) return this.updateSkill(e);
            if (e.remove) {
            ...
            if (e.master || e.follower) {
                M.show("master"), this.master = e.master || e.follower, this.is_follower = !!e.follower;
                var s = {};
                for (t = 0; t < e.items.length; t++) {
                    var i = e.items[t];
                    s[i.id] = i
                }
                this.skills = s, this.items = e.items, M.title(e.title), M.icon("book"), this.createSkillItems(e.items, s), e.limit && (this.is_follower ? M.footer("<span class='obj-money'>" + e.target + "目前的技能上限为<HIC>" + e.limit + "</HIC>级</span>") : M.footer("<span class='obj-money'>你目前的技能上限为<HIC>" + e.limit + "</HIC>级</span>"))
            }
        },
        show: function () {
            this.isShow || (this.element || (this.element = $('<div class="dialog-skills"></div >')), this.element.on("click", ".skill-item", this.item_click), this.element.appendTo(M.contentElement), this.element.removeClass("hide-item"), this.isShow = !0)
        },
        item_click: function () {
            var e = $(this),
                t = M.master.skills[e.attr("skid")];
            if (t) {
                var s = ["<div class='item-commands'>"];
                if (s.push('<span cmd="checkskill ' + t.id + " " + M.master.master + '">查看详细</span>'), s.push('<span cmd="xue ' + e.attr("skid") + " from " + M.master.master + '">学习</span>'), M.master.is_follower) {
                    var i = "dc " + M.master.master;
                    if (s.push('<span cmd="_confirm ' + i + " fangqi " + e.attr("skid") + '">遗忘</span>'), s.push('<span cmd="' + i + " lianxi " + e.attr("skid") + '">练习</span>'), t.can_enables)
                        for (var n = 0; n < t.can_enables.length; n++) {
                            var o = M.master.skills[t.can_enables[n]];
                    ...
                    if (t.enable_skill) {
                        var a = M.master.skills[t.enable_skill];
                        a ? s.push('<span cmd="' + i + " enable " + t.id + ' none">取消装备' + a.name + "</span>") : t.enable_skill = null
                    }
                }
                s.push("</div>"), M.master.element.find(".item-commands").remove(), $(s.join("")).appendTo(e)
            }
        }
    }, M.pack = {
        close: M.skills.close,
        ...
        updateitem: function (e) {
            if (this.items) {
                ...
                if (e.desc) {
                    ...
                    } else "store" == o ? n.push('<span cmd="_confirm qu ' + e.id + '">取出</span>') : n.push('<span cmd="_confirm buy ' + e.id + " from " + M.list.seller + '">购买</span>');
                    return n.push("</span>"), this.show_sub(n.join("")), !1
                }
                if (e.remove) {
                    var l = this.items;
                ...
                return e.name ? ((s = this.get_item(e.id)) ? (s.count = e.count, s.name = e.name) : this.items.push(e), !0) : !!e.max_item_count && (this.max_count = e.max_item_count, S((M.pack2.isShow ? M.pack2.target_name : "你") + "的背包容量扩充为" + this.max_count + "。"), !0)
            }
        },
        ...
        show_sub: function (e) {
            ...
            M.list.isShow && (t = M.list.rightElement), this.objelement = $("<pre class='obj-desc'>" + e + "</pre>").appendTo(t.parent()).on("click", function () {
        ...
        onData: function (e) {
            if (e) {
                if (e.items) this.eqs = e.eqs || [], this.money = e.money, this.items = e.items, this.max_count = e.max_item_count;
                    ...
                    if (M.pack2.isShow && !e.name) return M.pack2.onData(e);
                    if (0 == this.updateitem(e)) return
                }
                this.isShow ? (this.show_items(), this.show_moeny()) : (M.list.isShow && M.list.update_pack(e), M.trade.isShow && M.trade.update_pack(e))
            }
        },
        show_moeny: function () {
            this.isShow && (this.money ? M.footer("<div class='obj-money'>" + P(this.money)) : M.footer("<div class='obj-money'>" + (this.target_name || "你") + "身上没有任何银两</div>"))
        },
        show_items: function () {
            this.createItems(), this.create_eqs(), M.icon("briefcase");
            var e = this.target_name || "你";
            M.title(this.items.length ? e + "身上共有" + this.items.length + "/" + this.max_count + "件物品" : e + "身上没有任何东西")
        },
        ...
        show: function () {
            if (M.isShow || M.show(), this.objelement && (this.objelement.remove(), this.objelement = null, this.packElement && this.packElement.show()), this.isShow) return c(this.items ? "pack none" : "pack");
            this.isShow = !0, this.init_element(), this.packElement.on("click", ".obj-item", M.pack.item_click), this.eqElement.on("click", ".eq-item", M.pack.eqitem_click), this.element.appendTo(M.contentElement), this.items ? (c("pack none"), this.show_items()) : c("pack")
        },
        create_eqs: function () {
        ...
        createItems: function () {
            if (this.items) {
                for (var e = M.pack.sort_items(this.items), t = [], s = 0; s < this.max_count; s++) {
                    var i = e[s];
                    t.push('<div class="obj-item" oindex="'), t.push(i ? i.id : ""), t.push('">'), i && (t.push(i.name), 1 == this.show_type ? (t.push("<span class='obj-value'>"), t.push("每"), t.push(i.unit), t.push(P(i.value)), t.push("："), t.push(i.count), t.push(i.unit), t.push("</span>")) : 1 < i.count && (t.push("<span class='obj-value'>"), t.push(i.count), t.push(i.unit), t.push("</span>"))), t.push("</div>")
                }
        ...
        item_click: function () {
            var e = $(this),
                t = e.attr("oindex");
            if (t) {
                var s = M.pack.get_item(t);
                if (M.pack.element.find(".item-commands").remove(), s) {
                    var i = ["<span class='item-commands'>"];
                    i.push('<span cmd="checkobj ' + s.id + ' from item">查看</span>'), M.pack.create_item_command(s, i), i.push("</span>"), $(i.join("")).appendTo(e)
                }
            }
        },
        eqitem_click: function () {
            var e = $(this).find(".eq-name"),
                t = M.pack.eqs[e.attr("oindex")];
        ...
        createItems: M.pack.createItems,
        create_eqs: M.pack.create_eqs,
        init_element: M.pack.init_element,
        show_items: M.pack.show_items,
        updateitem: M.pack.updateitem,
        show_moeny: M.pack.show_moeny,
        show_sub: M.pack.show_sub,
        close: M.skills.close,
        hide: function () {
            this.element.remove(), this.isShow = !1
        },
        get_item: M.pack.get_item,
        create_item_command: M.pack.create_item_command,
        show: function () {
            M.isShow || M.show("pack2"), this.objelement && (this.objelement.remove(), this.objelement = null, this.packElement && this.packElement.show()), this.isShow || (this.isShow = !0, this.init_element(), this.packElement.on("click", ".obj-item", this.item_click), this.eqElement.on("click", ".eq-item", this.eqitem_click), this.element.appendTo(M.contentElement))
        },
        item_click: function () {
            var e = $(this),
                t = e.attr("oindex");
            if (t) {
                var s = M.pack2.get_item(t);
                if (M.pack2.element.find(".item-commands").remove(), s) {
                    var i = ["<span class='item-commands'>"];
                    i.push('<span cmd="' + M.pack2.command_before + " checkobj " + s.id + ' from item">查看</span>'), M.pack2.create_item_command(s, i), i.push("</span>"), $(i.join("")).appendTo(e)
                }
            }
        },
        eqitem_click: function () {
            var e = $(this).find(".eq-name"),
                t = M.pack2.eqs[e.attr("oindex")];
            t && c(M.pack2.command_before + " checkobj " + t.id + " from eq")
        }
    }, M.trade = {
        hide: function () {
            this.element.remove(), this.isShow = !1
        ...
        onData: function (e) {
            this.isShow || M.show("trade"), M.title("和" + e.name + "交易中");
            M.pack.items;
            this.trade_target = e.target, this.trade_list.length = 0, M.pack.items ? this.update_pack() : c("pack"), M.pack.isShow = !1, this.create_items(this.leftElement.empty(), this.trade_list, this.max_count)
        },
        update_pack: function (e) {
            this.create_items(this.rightElement.empty(), M.pack.items, M.pack.max_count)
        },
        max_count: 10,
        trade_list: [],
        show: function (e) {
            this.isShow || (M.init(), M.curItem = "trade", this.element || (this.element = $('<div class="dialog-list"><div class="obj-list"></div><div class="obj-list"></div></div >'), this.leftElement = $(this.element.children()[0]), this.rightElement = $(this.element.children()[1])), this.leftElement.on("click", ".obj-item", this.left_click), this.rightElement.on("click", ".obj-item", this.right_click), this.element.appendTo(M.contentElement.empty()), this.create_footer(), this.isShow = !0)
        },
        create_footer: function () {
            var e = [];
            e.push("<span class='footer-item trade_btn ok' for='1''>确定</span>"), e.push("<span class='footer-item  trade_btn' for='0''>取消</span>"), M.footer(e.join(""))
        },
        footerChanged: function (e) {
        ...
        create_items: function (e, t, s) {
            var i = [];
            t = M.pack.sort_items(t);
            for (var n = 0; n < s; n++) {
                var o = t[n];
                i.push('<div class="obj-item" oindex="'), i.push(o ? o.id : ""), i.push('"'), o ? (i.push(" oid='" + o.id + "'>"), i.push(o.name), 1 < o.count && (i.push("<span class='obj-value'>"), i.push(o.count), i.push(o.unit), i.push("</span>"))) : i.push("'>"), i.push("</div>")
            }
            e.html(i.join(""))
        },
        left_click: function () {
            ...
            if (e) {
                for (var t = null, s = 0; s < M.trade.trade_list.length; s++)
                    if (M.trade.trade_list[s].id == e) {
                        t = M.trade.trade_list[s];
                        break
                    } if (t) return M.trade.cancle_trade(t), !1
            }
        },
        ...
        right_click: function () {
            ...
            if (!e.is(".disabled")) {
                ...
                if (t) {
                    var s = M.pack.get_item(t);
                    if (s) return 1 < s.count ? F.Show_trade_add(s) : M.trade.add_trade(s), !1
                }
            }
        },
        ...
        updateitem: function (e) {
            if (e.store) {
                ...
                t ? t.count -= e.store : ((t = $.extend({}, s)).id = e.id, t.count = -e.store, M.pack.items.push(t)), s ? s.count += e.store : ((s = $.extend({}, t)).id = e.storeid, s.count = e.store, this.stores.push(s)), 0 == s.count && this.stores.Remove(s), 0 == t.count && M.pack.items.Remove(t)
            } else if (e.sell) {
                if (t = this.find_item(2, e.id)) return t.count -= e.sell, this.create_items(this.selllist, this.leftElement, 2, this.selllist.length)
            }
            this.isstore && (this.create_items(this.stores, this.leftElement, 3, this.max_store_count), M.title("你的仓库中有" + this.stores.length + "/" + this.max_store_count + "件物品")), this.update_pack(), null != e.money && this.show_footer(e.money)
        },
        ...
        onData: function (e) {
            if (this.show(), e) {
                ...
                e.selllist && (this.isstore = !1, this.gongji = t, this.money_name = null, e.gongji ? this.money_name = "门派功绩" : e.jungong && (this.money_name = "军功"), this.selllist = e.selllist, this.create_items(e.selllist, this.leftElement, 2, e.selllist.length), M.titleElement.html(e.title), M.icon("shopping-cart")), this.update_pack(), e.stores && (this.create_items(e.stores, this.leftElement, 3, e.max_store_count), this.isstore = !0, this.stores = e.stores, M.titleElement.html("你的仓库中有" + e.stores.length + "/" + e.max_store_count + "件物品"), this.max_store_count = e.max_store_count, M.icon("lock")), t && (this.gongji = t, this.show_footer(t)), e.seller && (this.seller = e.seller)
            }
        },
        show: function (e) {
            M.isShow && "list" == M.curItem || M.show("list"), this.rightElement && (this.rightElement.show(), M.pack.objelement && M.pack.objelement.remove()), this.isShow || (this.element || (this.element = $('<div class="dialog-list"><div class="obj-list"></div><div class="obj-list"></div></div >'), this.leftElement = $(this.element.children()[0]), this.rightElement = $(this.element.children()[1])), this.element.on("click", ".obj-item", M.list.item_click), this.element.appendTo(M.contentElement.empty()), this.isShow = !0)
        },
        show_footer: function (e) {
            if (0 < (e = this.money_name ? this.gongji : e)) {
                var t = this.money_name ? "你目前有" + e + "<hiy>" + this.money_name + "</hiy>" : "你身上有" + P(e);
                M.footerElement.html("<div class='obj-money'>" + t + "<span cmd='sell all'>清理包裹</span></div>")
            } else {
        ...
        update_pack: function () {
            var e = M.pack.items;
            e ? (this.create_items(e, this.rightElement, 1, M.pack.max_count), this.show_footer(M.pack.money)) : c("pack")
        },
        create_items: function (e, t, s, i) {
            var n = [],
                o = e;
            1 != s && 3 != s || (o = M.pack.sort_items(e));
            for (var a = 0; a < i; a++) {
                var l = o[a];
                n.push('<div class="obj-item" '), l ? (n.push('obj="'), n.push(l.id), n.push('" otype="'), n.push(s), n.push('">'), n.push(l.name), n.push("<span class='obj-value'>"), 2 == s ? (n.push("每"), n.push(l.unit), n.push(this.money_name ? l.value + "<hiy>" + this.money_name + "</hiy>" : P(l.value)), -1 == l.count ? n.push("：大量现货") : (n.push("：剩余"), n.push(l.count), n.push(l.unit))) : 1 != s || this.isstore ? 1 < l.count && (n.push(l.count), n.push(l.unit)) : l.value ? (n.push("每"), n.push(l.unit), n.push(P(l.value)), n.push("："), n.push(l.count), n.push(l.unit)) : n.push("不可出售"), n.push("</span>")) : n.push('">'), n.push("</div>")
            }
            t.html(n.join(""))
        },
        item_click: function () {
            var e = $(this),
                t = e.attr("obj"),
                s = e.attr("otype"),
                i = M.list.find_item(s, t);
            if (i) {
                var n = ["<div class='item-commands'>"];
                M.list.isstore ? 3 == s ? (n.push('<span cmd="checkobj ' + t + ' from store">查看</span>'), n.push('<span cmd="_confirm qu ' + t + '">取出</span>')) : 1 == s && (n.push('<span cmd="checkobj ' + t + ' from item">查看</span>'), n.push('<span cmd="_confirm store ' + i.count + " " + t + '">存到仓库</span>')) : 2 == s ? (n.push('<span cmd="checkobj ' + t + " from " + M.list.seller + '">查看</span>'), i.count && n.push('<span cmd="_confirm buy ' + i.count + " " + t + " from " + M.list.seller + '">购买</span>')) : 1 == s && (n.push('<span cmd="checkobj ' + t + ' from item">查看</span>'), n.push('<span cmd="_confirm sell ' + i.count + " " + t + " to " + M.list.seller + '">卖掉</span>')), n.push("</div>"), M.list.element.find(".item-commands").remove(), $(n.join("")).appendTo(e)
            }
        }
    }, M.channel = {
        footer: [
        ...
        show: function () {
            ...
            else if (!M.channel.isShow) {
                M.select("channel"), M.icon("comment"), M.title(""), M.footer("");
                for (var e = 0; e < M.channel.footer.length; e++) {
                    var t = $("<span class='footer-item channel-item' for='" + M.channel.footer[e][1] + "'>" + M.channel.footer[e][0] + "</span>").appendTo(M.footerElement);
                    0 == e && t.addClass("select")
                }
                M.contentElement.html("").append(T.ChannelElement.addClass("channel-dialog")), M.channel.isShow = !0, M.channel.scrollBottom()
            }
        },
        hide: function () {
            M.channel.footerChanged(""), T.ChannelElement.removeClass("channel-dialog").insertBefore(u), this.scrollBottom(), this.isShow = !1
        },
        close: function () {
            this.hide()
        },
        scrollBottom: function () {
            M.channel.isScroll && (T.ChannelElement[0].scrollTop = 999999)
        },
        isBottom: function () {
            return !0
        },
        endScroll: function () {
            M.channel.isScroll = !1
        },
        beginScroll: function () {
            M.channel.isScroll = !0
        },
        footerChanged: function (e) {
            if (M.channel.select_item != e) {
                M.channel.select_item = e;
        ...
        show: function () {
            if (this.init(), !this.isShow) {
                this.footerChanged("setting"), M.icon("cog"), M.title("设置"), M.footerElement.empty();
                for (var e = 0; e < this.footer.length; e++) {
                    var t = $("<span class='footer-item' for='" + this.footer[e][1] + "'>" + this.footer[e][0] + "</span>").appendTo(M.footerElement);
                    0 == e && t.addClass("select")
                }
                this.isShow = !0
            }
        ...
        footerChanged: function (e) {
            this.selectitem && this.selectitem.remove(), this.selectitem = this[e + "Element"], "setting" == e ? (this.selectitem.on("click", ".switch", this.switchClick), this.selectitem.on("click", ".color-item", this.colorClick)) : "custom" == e ? (this.selectitem.on("click", ".switch", this.switchClick), this.selectitem.on("click", ".setting-ok", this.save_custom)) : this.selectitem.on("click", ".help-item", this.helpClick), this.selectitem.appendTo(M.contentElement)
        },
        helpClick: function () {
            ...
            switch (e) {
                ...
                case "toserver":
                    ...
                    break;
                default:
                    M.setting.show_help(e)
            }
        },
        show_help: function (e) {
            U.Get("doc/" + e + ".html", function (e) {
        ...
        save_custom: function () {
            if ($(".dialog-custom>.setting-item[for='auto_pfm']>.switch").is(".on")) {
                ...
                if (300 < e.length) return S("<hir>你设置的出招过长。</hir>");
                q.save("auto_pfm", e)
            }
            ...
            if ($(".dialog-custom>.setting-item[for='auto_work']>.switch").is(".on")) {
                var e;
                if ((e = $("#auto_work").val()) && 300 < e.length) return S("<hir>你设置的过长。</hir>");
                q.save("auto_work", e || 1)
            }
            S("<hic>设置已保存。</hic>")
        },
        get_pfms: function (e) {
            if (!j.Skills) return S("<hir>你没有可用的绝招设置。</hir>");
            for (var t = [], s = 0; s < j.Skills.length; s++) 0 < t.length && t.push(","), t.push(j.Skills[s].id);
        ...
        switchClick: function (e) {
            ...
            switch (t.is(".on") ? (t.removeClass("on"), t.find(".switch-text").html("关")) : (t.addClass("on"), t.find(".switch-text").html("开"), i = 1), s) {
                case "auto_pfm":
                case "auto_pfm2":
                    i ? ($("#" + s).show(), M.setting.get_pfms(s), q[s] = 0) : ($("#" + s).hide(), q.save(s, 0));
                    break;
                case "auto_work":
                    i ? $("#" + s).show() : ($("#" + s).hide(), q.save(s, 0));
                    break;
                default:
                    q.save(s, i)
            }
        ...
        onData: function (e) {
            ...
            M.title("任务列表"), M.icon("exclamation-sign"), this.items = e.items, this.create_items()
        },
        show: function () {
            this.element || (this.element = $("<div class='dialog-tasks'></div>")), c("tasks"), this.isShow || (this.element.appendTo(M.contentElement), M.footer(""), this.isShow = !0)
        },
        ...
        onData: function (e) {
            return e.close ? M.hide() : e.tops ? (e.top ? this.show_desc("你目前在第" + e.top + "名") : this.show_desc("你目前没有上榜"), this.create_tops(e.tops, e)) : e.weapons ? (this.show_desc(""), this.create_weapons(e.weapons)) : e.scores ? (this.show_desc("你目前的评分：" + e.score), this.create_scores(e.scores)) : void 0
        },
        ...
        show: function () {
            this.selectedItem || (this.selectedItem = this.footers[0]), c("stats " + this.selectedItem.cmd), this.element || (this.element = $("<div class='dialog-stats'></div>")), this.isShow || (this.create_footer(), M.icon("stats"), M.contentElement.html(this.element), this.element.on("click", ".top-item", this.itemClick), this.isShow = !0)
        },
        create_footer: function () {
            ...
            e.push("<span class='stats-span'></span>"), M.footer(e.join(""))
        },
        show_desc: function (e) {
            M.footerElement.find(".stats-span").html(e)
        },
        footerChanged: function (e) {
            var t = this.footers[e];
            t != this.selectedItem && (this.selectedItem = t, M.title(this.selectedItem.name), c("stats " + this.selectedItem.cmd))
        },
        itemClick: function () {
            var e = $(this),
            ...
            s ? (n.push('<span cmd="look1 ' + t + '">查看</span>'), !M.stats.top || t < M.stats.top ? n.push('<span cmd="challenge ' + t + '">挑战</span>') : t == M.stats.top && n.push('<span cmd="stats copy">更新应战状态</span>'), n.push('<span cmd="reward top ' + t + '">查看奖励</span>')) : i ? (n.push('<span cmd="stats weapon ' + t + '">查看</span>'), n.push('<span cmd="reward weapon ' + t + '">查看奖励</span>')) : (n.push('<span cmd="stats score ' + t + '">查看</span>'), n.push('<span cmd="reward score ' + t + '">查看奖励</span>')), n.push("</div>"), M.stats.element.find(".item-commands").remove(), $(n.join("")).appendTo(e)
        }
    }, M.jh = {
        unlock: 0,
        close: function () {
        ...
        onData: function (e) {
            if (e.close) return M.isShow && M.hide();
            if (e.desc) {
                if ("fb" == e.t) {
        ...
        select: function (e) {
            ...
            if (t.length && !t.is(".selected")) {
                ...
                i / 2 < s && (s = (i - t.height()) / 2, M.jh.listElement[0].scrollTop = s), M.jh.selectedItem && M.jh.selectedItem.removeClass("selected"), M.jh.selectedItem = t, M.jh.selectedItem.addClass("selected")
            }
        },
        ...
        show: function () {
            this.isShow || (this.element || (this.element = $("<div class='dialog-fb'><div class='fb-left'></div><div class='fb-right'></div></div>")), this.listElement = this.element.find(".fb-left").on("click", ".fb-item,.fam-item", this.item_click), this.descElement = this.element.find(".fb-right"), M.title("江湖"), M.icon("home"), this.element.appendTo(M.contentElement), this.create_footer(), this.fbs ? c("jh fb lock") : c("jh"), this.isShow = !0)
        },
        selected_item: 0,
        footers: ["门派", "副本"],
        create_footer: function () {
            if (this.fbs) {
                ...
                $(e.join("")).appendTo(M.footerElement)
            }
        },
        item_click: function () {
            var e = $(this);
            if (!e.is(".selected")) {
                ...
                if (e.is(".fb-item")) {
                    var s = M.jh.fbs[t];
                    s.desc ? M.jh.show_fbdesc(s) : c("jh fb " + t)
                } else {
                    var i = M.jh.families[t];
                    i.desc ? M.jh.show_famdesc(i) : c("jh fam " + t)
                }
                M.jh.selectedItem && M.jh.selectedItem.removeClass("selected"), M.jh.selectedItem = e, M.jh.selectedItem.addClass("selected")
            }
        },
        ...
        show_moeny: function () {
            this.isShow && (window.isios ? M.footer("<div class='dialog-shop-footer'>你身上有<hij> " + this.cash_money + " 元宝</hij></div>") : M.footer("<div class='dialog-shop-footer'>你身上有<hij> " + this.cash_money + " 元宝</hij><span cmd='transmoney'>账号转入</span><span cmd='pay'>充值</span><span cmd='tg'>推广</span></div>"))
        },
        show: function (e) {
            this.element || (this.element = $("<div class='dialog-shop'></div>")), this.isShow || (M.title("商品列表"), M.icon("shopping-cart"), this.isShow = !0, this.element.appendTo(M.contentElement), this.selllist ? this.show_moeny() : c("shop"))
        },
        ...
        show: function (e) {
            this.unRead = 0, this.showUnread(), this.isShow || (this.isShow = !0, M.title("消息"), M.icon("envelope"), this.create_footer(), this.footerChanged(this.selected_item), this.isLoad || c("message"), this.isLoad = !0, this.element.on("click", ".message-item", this.showMessageDetail))
        },
        footers: ["消息", "队伍", "关系", "帮派"],
        footerElements: ["message", "team", "relation", "party"],
        create_footer: function () {
            for (var e = [], t = 0; t < this.footers.length; t++) e.push("<span class='footer-item" + (t == this.selected_item ? " select" : "") + "' for='" + t + "''>" + this.footers[t] + "</span>");
            M.footer(e.join("")), this.showChild()
        },
        footerChanged: function (e) {
            e != this.selected_item && (this.selected_item = e, this.showChild())
        },
        showChild: function () {
            var e = M[this.footerElements[this.selected_item]];
            e.element || (e.element = e.createElement()), M.contentElement.html(e.element), this.selectedChild != e && (this.selectedChild && this.selectedChild.close(), e.show(), this.selectedChild = e)
        },
        ...
        showMessageDetail: function () {
            ...
            e && (c("message " + e), M.message.element.addClass("detail"))
        },
        ...
        show: function () {
            c("party load"), this.isShow = !0, M.title(""), this.element.on("click", ".party-role", this.show_commands), M.icon("flag")
        },
        ...
        get_role: function (e) {
            if (this.roles)
                for (var t = 0; t < this.roles.length; t++)
                    if (this.roles[t].id == e) return this.roles[t]
        },
        onData: function (e) {
            if (e.name) {
                var t = e,
                    s = [];
                s.push("<div class='party-title'><hio>"), s.push(t.name), s.push("</hio><span class='party-count'><nor>(" + e.roles.length + "/" + this.level_roles[e.level] + ")</nor></span>"), s.push("</div>"), t.notice && (s.push("<div class='party-notice'><hio><span class='glyphicon glyphicon-volume-up'></span>"), s.push(t.notice), s.push("</hio></div>")), s.push("<div class='party-roles'>");
                for (var i = 0; i < t.roles.length; i++) {
                    var n = t.roles[i];
                    n.id == T.player && (this.level = n.level), s.push("<div class='party-role' roleid='" + n.id + "'>"), s.push("<span class='role-level'>"), s.push(this.levels[n.level]), s.push("</span>"), s.push("<span class='role-name'>"), s.push(n.name), s.push("</span>"), s.push("<span class='role-sc'>"), s.push(n.sc), s.push("</span>"), s.push("</div>")
                }
                s.push("</div>"), this.roles = e.roles, this.element.html(s.join(""))
            } else this.element.html("<wht>你没有加入帮派<wht>")
        },
        show_commands: function () {
            var e = M.party.get_role($(this).attr("roleid"));
            if (e) {
                var t = ["<div class='item-commands'>"];
                e.id == T.player ? (t.push('<span cmd="party out">退出帮派</span>'), 1 == M.party.level && t.push('<span cmd="party dissmiss">解散</span>')) : (e.level > M.party.level - 1 && 2 < e.level && t.push('<span cmd="party uplevel ' + e.id + '">提升为' + M.party.levels[e.level - 1] + "</span>"), e.level > M.party.level && e.level < 5 && t.push('<span cmd="party downlevel ' + e.id + '">降级为' + M.party.levels[e.level + 1] + "</span>"), 1 == M.party.level && 2 == e.level && t.push('<span cmd="party trans ' + e.id + '">让位</span>'), e.level > M.party.level && t.push('<span cmd="party remove ' + e.id + '">开除</span>'), e.online && t.push('<span cmd="team add ' + e.id + '">邀请组队</span>')), 1 != t.length && (t.push("</div>"), M.party.element.find(".item-commands").remove(), $(t.join("")).appendTo(this))
            }
        },
        close: function () {
            this.element.remove(), this.isShow = !1
        ...
        show: function () {
            c("team"), this.isShow = !0, M.title("队伍"), this.element.on("click", ".team-item", this.clickItem), M.icon("list")
        },
        items: [],
        onData: function (e) {
        ...
        clickItem: function () {
            var e = $(this),
                t = M.team.items[e.attr("index")];
            if (t) {
                var s = ["<div class='item-commands'>"];
                s.push('<span cmd="look3 ' + t.id + '">查看</span>');
                var i = M.team.items[0].id == T.player;
                i && t.id != T.player ? s.push('<span cmd="team remove ' + t.id + '">移出队伍</span>') : t.id == T.player && s.push('<span cmd="team out ' + t.id + '">退出队伍</span>'), i && t.id == T.player && s.push('<span cmd="team set">更改分配方式</span>'), s.push("</div>"), M.team.element.find(".item-commands").remove(), $(s.join("")).appendTo(e)
            }
        }
    }, M.friend = {
        show: function () {
    ...
    var q = {
            ...
            set_prop: function (e, t) {
                switch (e) {
                    ...
                    case "off_hp":
                        ...
                        break;
                    case "combat_wrap":
                        t ? $(".combat-commands").addClass("combat-wrap") : $(".combat-commands").removeClass("combat-wrap"), console.log($(".combat-commands").height());
                        break;
                    ...
                    case "show_hp":
                        ...
                        break;
                    case "show_hpnum":
                        T.cur_room && T.items(T.cur_room);
                        break;
                    case "show_damage":
                        $(".item-damage").remove();
                        break;
                    case "fullscreen":
                        t ? q.launchFullScreen() : q.exitFullscreen();
                        break;
                    case "show_command":
                        T.itemsElement.find(".item-commands").remove();
                        break;
        ...
        R = "万俟司马上官欧阳夏侯诸葛闻人东方赫连皇甫尉迟公羊澹台公冶宗政濮阳淳于单于太叔申屠公孙仲孙轩辕令狐锺离宇文长孙慕容鲜于闾丘司徒司空丌官司寇子车颛孙端木巫马公西乐正公良拓拔夹谷谷梁梁丘左丘东门西门",
    ...
    var F = {
            ...
            Close: function () {
                F.isShow && (F.element.css("bottom", -F.Parameter.height), setTimeout(function () {
                    F.element.hide()
                }, 200), F.isShow = !1)
            },
            Init: function () {
                this._init || (this.element = $(".dialog-confirm"), this.content = this.element.find(".dialog-content"), this.btn = this.element.find(".dialog-btn"), this.element.on("click", ".btn-ok", function (e) {
                    return F.Parameter.onOK(), F.Close(), !1
                }), this.element.on("click", ".btn", function (e) {
                    var t = F.max_count || 1e3,
                        s = $(e.target),
            ...
            Show_shop: function (e) {
                var s = e[2];
                if (s) {
                    var t = M.shop.selllist[s];
                    t && this.Show({
                        content: this.get_countelement(),
                        btn_text: "购买" + t.name,
                        onOK: function () {
                            var e = F.count_element.find("input"),
                                t = parseInt(e.val());
                            "NaN" == t.toString() && (t = 0), t && (t > F.max_count && (t = F.max_count), c("shop " + s + " " + t))
                        }
                    })
                }
            },
                ...
                if (i) {
                    ...
                    this.Show({
                        ...
                        onOK: function () {
                            var e = F.count_element.find("input"),
                                t = parseInt(e.val());
                            "NaN" == t.toString() && (t = 0), t && (t > F.max_count && (t = F.max_count), c("buy " + t + " " + i + " from " + s[5]))
                        }
                    })
                }
            },
            Show_greet: function (e) {
                this.Show({
                    ...
                    onOK: function () {
                        var e = F.count_element.find("input"),
                            t = parseInt(e.val());
                        "NaN" == t.toString() && (t = 0), t && (t > F.max_count && (t = F.max_count), c("greet " + t))
                    }
                })
            },
            Show_sell: function (s) {
                ...
                i && this.Show({
                    ...
                    onOK: function () {
                        var e = F.count_element.find("input"),
                            t = parseInt(e.val());
                        "NaN" == t.toString() && (t = 0), t && (t > F.max_count && (t = F.max_count), c("sell " + t + " " + i + " to " + s[5]))
                    }
                })
            },
            Show_store: function (e) {
                ...
                if (s) return 1 == e[2] ? c("store " + s) : void this.Show({
                    ...
                    onOK: function () {
                        var e = F.count_element.find("input"),
                            t = parseInt(e.val());
                        "NaN" == t.toString() && (t = 0), t && (t > F.max_count && (t = F.max_count), c("store " + t + " " + s))
                    }
                })
            },
            Show_fenjie: function (e, t) {
                var s = e[2];
                if (s) {
                    var i = M.pack.isShow ? M.pack.get_item(s) : M.pack2.get_item(s);
                ...
                if (s) {
                    ...
                    if (t) return 1 == t.count ? c("qu 1 " + s) : void this.Show({
                        ...
                        onOK: function () {
                            var e = F.count_element.find("input"),
                                t = parseInt(e.val());
                            "NaN" == t.toString() && (t = 0), t && (t > F.max_count && (t = F.max_count), c("qu " + t + " " + s))
                        }
                    })
                }
            },
            Show_drop: function (s, i) {
                var n = s[3];
                if (n) {
                    var e = M.pack.isShow ? M.pack.get_item(n) : M.pack2.get_item(n);
                    e && this.Show({
                        ...
                        onOK: function () {
                            ...
                            var e = F.count_element.find("input"),
                                t = parseInt(e.val());
                            "NaN" == t.toString() && (t = 0), t > F.max_count && (t = F.max_count), c(i + "drop " + t + " " + n)
                        }
                    })
                }
            },
                ...
                if (i) {
                    ...
                    if (t) return 1 == t.count ? c(s + "give " + T.player + " 1 " + i) : void this.Show({
                        ...
                        onOK: function () {
                            var e = F.count_element.find("input"),
                                t = parseInt(e.val());
                            "NaN" == t.toString() && (t = 0), t > F.max_count && (t = F.max_count), c(s + "give " + T.player + " " + t + " " + i)
                        }
                    })
                }
            },
                ...
                i && this.Show({
                    ...
                    onOK: function () {
                        ...
                        if (!("NaN" == t.toString() || t <= 0)) {
                            t > i.count && (t = i.count);
                            var s = U.Clone(i);
                            s.count = t, console.log(s), M.trade.add_trade(s)
                        }
                    }
                })
            },
            Show_fangqi: function (e, t) {
                var s = e[2];
                if (s) {
                    var i = t ? M.master.skills[s] : M.skills.skills[s];
                    i && this.Show({
                        ...
                        if (n) {
                            ...
                            this.Show({
                                ...
                                onOK: function () {
                                    var e = F.count_element.find("input"),
                                        t = parseInt(e.val());
                                    "NaN" == t.toString() && (t = 0), t && (t > F.max_count && (t = F.max_count), c(s + "combine " + i + " " + t))
                                }
                            })
                        }
                    }
        ...
        U = {
            ...
            to_c: function (e) {
                ...
                for (var t = "", s = 0, i = 0; e;) {
                    var n = e % 10;
                    s && (s % 4 == 0 && 3 != i ? (t = U.C_STR3[s / 4] + t, i = 3) : n && 2 != i && (t = U.C_STR2[s % 4] + t, i = 2)), n ? ((1 != n || 10 < e || s % 4 != 1) && (t = U.C_STR[n] + t), i = 1) : 1 == i && (t = U.C_STR[n] + t, i = 0), e = parseInt(e / 10), s++
                }
                return t
            },
            Get: function (e, t, s) {
                if (e) {
                    var i = [];
                    if ($.isPlainObject(t)) {
                        for (var n in t) t[n] && i.push(n + "=" + U.encode(U.Json2Str(t[n])));
                        e = e + "?" + i.join("&")
                    } else if ($.isFunction(t)) s = t;
                    else if ($.isArray(t)) {
                        for (var o = 0; o < t.length; o++) i.push(U.encode(U.Json2Str(t[o])));
                        e = e + "/" + i.join("/")
                    }
                    var a = {
                        url: "/" + e,
            ...
            Post: function (e, t, s) {
                var i, n = [];
                if ($.isPlainObject(t)) {
                    for (var o in t) t[o] && n.push(o + "=" + U.Json2Str(t[o]));
                    i = n.join("&")
                } else {
                    if (!t.length) return;
                    for (var a = 0; a < t.length; a++) n.push(U.Json2Str(t[a]));
                    i = U.Json2Str2(n)
                }
                var l = {
                    url: "/" + e,
            ...
            ToDate: function () {
                ...
                if (1 != arguments.length) return new Date(arguments[0], arguments[1], arguments[2]);
    ...
    window.WebSocket = null, G.prototype.Connect = function (e) {
        try {
            location.protocol;
            this.ws = new B("ws://" + this.IP + ":" + this.Port), this.ws.onopen = this.OnConnect, this.ws.onclose = this.OnClose.bind(this), this.ws.onerror = this.OnError, this.ws.onmessage = this.OnReceived.bind(this), this.index = N++
        } catch (e) {
            this.OnError && this.OnError(e)
        }
    }, G.prototype.OnReceived = function (e) {
        if (e && e.data) {
            var t = e.data;
            if ("{" == t[0] || "[" == t[0]) {
        ...
        this.ws.close()
    }, G.prototype.Connected = function () {
        return this.ws && 1 == this.ws.readyState
    };
    var L = {};
    L.UserAPI = {
        Login: function (e, t, s) {
            return U.Post("UserAPI/Login", {
                code: e,
                pwd: t
            }, s)
        },
        Regist: function (e, t) {
            return U.Post("UserAPI/Regist", {
                user: e
            }, t)
        },
        Enter: function (e, t) {
        ...
        ChangePassword: function (e, t, s, i) {
            return U.Post("UserAPI/ChangePassword", {
                oldpwd: e,
                pwd: t,
                no: s
            }, i)
        ...
        LoginOut: function (e) {
            return U.Get("UserAPI/LoginOut", e)
        },
        GetRoles: function (e, t) {
            return U.Get("UserAPI/GetRoles", [e], t)
        },
        AddRole: function (e, t) {
            return U.Post("UserAPI/AddRole", {
                player: e
            }, t)
        },
        GetUser: function (e) {
            return U.Get("UserAPI/GetUser", e)
        },
        Search: function (e, t, s, i) {
            return U.Get("UserAPI/Search", [e, t, s], i)
        },
        ResetPassword: function (e, t) {
            return U.Get("UserAPI/ResetPassword", [e], t)
        },
        RecoverUser: function (e, t) {
            return U.Get("UserAPI/RecoverUser", [e], t)
        },
        LoadPlayer: function (e, t, s) {
            return U.Get("UserAPI/LoadPlayer", [e, t], s)
        },
        GetPhone: function (e) {
            return U.Get("UserAPI/GetPhone", e)
        },
        BindPhone: function (e, t, s) {
            return U.Post("UserAPI/BindPhone", {
                code: e,
                no: t
            }, s)
        },
        SendValidateCode: function (e, t) {
            return U.Get("UserAPI/SendValidateCode", [e], t)
        },
        ResetPasswordByPhone: function (e, t, s, i, n) {
            return U.Post("UserAPI/ResetPasswordByPhone", {
                name: e,
                phone: t,
                vcode: s,
                pwd: i
        ...
        NewServer: function (e) {
            return U.Get("UserAPI/NewServer", e)
        },
        GetServer: function (e) {
            return U.Get("Game/GetServer", e)
        }
    }
}();